...
(function(w, i) {
        ...
        if (! (o && d !== "br") && (!Y || !(A < 3 && d !== "p")) && !(Pa && A < 9)) {
            ...
            f(c).keydown(function(f) {
                if (! (f.which != 13 || f.shiftKey || f.ctrlKey || f.altKey)) {
                    b.cmd.selection();
                    var j = a(b.cmd.range);
                    j == "marquee" || j == "select" || (d === "br" && !e[j] ? (f.preventDefault(), b.insertHtml("<br />" + (o && A < 9 ? "": "\u200b"))) : g[j] || Q(c, "formatblock", "<span>"))
                }
            });
            f(c).keyup(function(e) {
                ...
                } else if (b.cmd.selection(), e = a(b.cmd.range), !(e == "marquee" || e == "select")) if (g[e] || Q(c, "formatblock", "<span>"), e = b.cmd.commonAncestor("div")) {
                    for (var j = f(""), k = e[0].firstChild; k;) {/*enter 另起一行时，行间距会拉很大，而且改不了*/
                        var n = k.nextSibling;
                        j.append(k);
                        k = n
                    }
    ...
    if (!w.KindEditor) {
        ...
        f.options = {
            ...
            minChangeSize: 50,
            zIndex: 811213,
            items: ["source", "|", "undo", "redo", "|", "preview", "print", "template", "code", "cut", "copy", "paste", "plainpaste", "wordpaste", "|", "justifyleft", "justifycenter", "justifyright", "justifyfull", "insertorderedlist", "insertunorderedlist", "indent", "outdent", "subscript", "superscript", "clearhtml", "quickformat", "selectall", "|", "fullscreen", "/", "formatblock", "fontname", "fontsize", "|", "forecolor", "hilitecolor", "bold", "italic", "underline", "strikethrough", "lineheight", "removeformat", "|", "image", "multiimage", "flash", "media", "insertfile", "table", "hr", "emoticons", "baidumap", "pagebreak", "anchor", "link", "unlink", "|", "about","replace"],
        ...
        E(oa, {
            ...
            select: function(a) {
                ...
                b.removeAllRanges(),
                b.addRange(j),
                /*g !== document && (c = f(j.endContainer).pos(), h.scrollTo(c.x, c.y));进行复制，粘贴，改变字体等操作，页面会向上移动*/
                h.focus();
                return this
            },
            wrap: function(a) {
            ...
            createyoutube: function(url) {
                var self = this,
                doc = self.doc,
                range = self.range;
                self.select();
				var html = '[youtube]' + Dyoutube(url) + '[/youtube]<br/>';
				var p1 = /width="([\d]+)"/;
				var w = url.match(p1);
				p1 = /height="([\d]+)"/;
				var h = url.match(p1);
				p1 = /src="(https:\/\/www.youtube.com[^"]+)"/;
				var u = url.match(p1);
				if(w && h && u){
					var html = '<iframe width="'+w[1]+'" height="'+h[1]+'" src="'+u[1]+'" frameborder="0" allowfullscreen></iframe><br/>';	
				}else{
					/*var html = '';*/
				}
